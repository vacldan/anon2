        ...
        for m in PAIR_RE.finditer(text_no_titles):
            ...
            if re.search(r'\b(výrobce|model|značka|inventář|výrobek|položk)', pre+post, re.IGNORECASE):
                if (normalize_for_matching(f_tok) in SURNAME_BLACKLIST or 
                    normalize_for_matching(l_tok            # Odebrat logiku pro anonimizaci jmen mimo knihovnu
            # Jména, která nejsou v knihovně, se neanonimizují) or CTX_ROLE.search(pre+post) or CTX_LABEL.search(pre+post)
            if (has_ctx
